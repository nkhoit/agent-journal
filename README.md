...
rejected. Archived spaces remain readable but reject new appends. Schema 11 is a
...
public access. Independently registered principals can post, fetch their durable
inbox and acknowledge items without an adapter. Optional-client conversion and
legacy delivery retirement remain slice 4. This intermediate checkpoint is not
deployment-ready: legacy custody and new inbox receipts are deliberately independent.

`aj inbox` returns one bounded page; follow its cursor to the end, then restart
without a cursor for pending retries. `aj inbox-ack --item ID` ends reminders
without implying runtime delivery or completion. A fixed first-page upper sequence
bound prevents continuous arrivals from extending a pass. Intentional older-backup
restore may lose later acknowledgments and repeat reminders under explicit operator
loss approval; ordinary retries and restarts preserve the first timestamp.
...
- **Inbox item** — stable record/recipient identity with recipient-local sequence and a nullable first acknowledgment timestamp.
...
The principal inbox is fetch then acknowledge, with no claim or runtime processing
guarantee. The following legacy adapter path remains only until slice 4 and never
changes inbox acknowledgment:
...
| OpenAPI 3.1 contract and S0 gate | Executable: validates the exact 35-path/37-operation surface, 79 fixture mappings, contract rules, fixture parsing, operation coverage, and deterministic contract mutations |
...
| Principal inbox | Atomic attention items, fixed-bound authenticated pagination, idempotent bodyless acknowledgment, typed client/CLI, and private receipt-only status; legacy custody never acknowledges items |
