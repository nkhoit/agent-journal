...
//!
//! The client deliberately uses only the supported authenticated Runs API. A
...
impl HermesRuntime {
    ...
    pub fn new(base_url: &str, api_key: impl Into<String>) -> RuntimeResult<Self> {
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|_| RuntimeError::RuntimeUnavailable(UNAVAILABLE.into()))?;
        let runtime = Self {
            client,
            base,
            api_key,
    ...
    fn preflight(&self) -> RuntimeResult<()> {
    ...
    fn request(
        ...
        if authenticated {
            let value = HeaderValue::from_str(&format!("Bearer {}", self.api_key))
                .map_err(|_| RuntimeError::RuntimeRejected)?;
            request = request.header(AUTHORIZATION, value);
        }
        if let Some(key) = idempotency_key {
            let value = HeaderValue::from_str(key).map_err(|_| RuntimeError::RuntimeRejected)?;
    ...
    fn ensure_session(&self, session_id: &str) -> RuntimeResult<()> {
...
impl Runtime for HermesRuntime {
    fn inject(&self, route: &Route, envelope: &Envelope, rendered: &str) -> RuntimeResult<String> {
        ...
        // in central records, and is sent only to Hermes.
        let idempotency_key = idempotency_key(&envelope.inbox_item_id)?;
...
fn validate_base_url(base_url: &str) -> RuntimeResult<Url> {
    let base = Url::parse(base_url).map_err(|_| RuntimeError::RuntimeRejected)?;
...
fn validate_session_id(session_id: &str) -> RuntimeResult<()> {
...
fn idempotency_key(inbox_item_id: &str) -> RuntimeResult<String> {
    let key = format!("agent-journal:{inbox_item_id}");
...
fn read_body(response: Response) -> RuntimeResult<Vec<u8>> {
    if response
        .content_length()
...
fn discard_body(response: Response) -> RuntimeResult<()> {
    let _ = read_body(response)?;
    Ok(())
}

fn status_error(status: u16) -> RuntimeError {
    if status == 429 || (500..=599).contains(&status) {
        RuntimeError::RuntimeUnavailable(UNAVAILABLE.into())
    } else {
        RuntimeError::RuntimeRejected
    }
}

fn invalid_response() -> RuntimeError {
...
mod tests {
    ...
    fn idempotency_key_is_stable_visible_ascii_and_bounded() {
        assert_eq!(idempotency_key("item-42").unwrap(), "agent-journal:item-42");
