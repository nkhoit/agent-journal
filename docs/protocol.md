...
All request and response decoding must pass through the duplicate-key-rejecting decoder before typed deserialization. Request DTOs reject unknown fields and distinguish an omitted optional field from an explicit `null` when the OpenAPI schema does not permit null.
